...
import type { Track, Clip, VideoTrack, AudioTrack, MediaClip } from '../types';
...
export const MediaPool: React.FC<MediaPoolProps> = ({ onFilesChange, tracks }) => {
  ...
  const allMediaClips: MediaClip[] = mediaTracks.flatMap(track => track.clips);
    ...
    <div className="bg-promptus-surface rounded-lg p-4 flex flex-col h-full border border-promptus-border">
      ...
      {allMediaClips.length > 0 ? (
        <div className="flex-grow overflow-y-auto space-y-2 pr-2">
            {allMediaClips.sort((a,b) => a.start - b.start).map(clip => (
      ...
      <div
        ...
        onDrop={handleDrop}
        className={`relative mt-4 flex-shrink-0 flex flex-col items-center justify-center border-2 border-dashed rounded-md transition-colors h-24 ${
